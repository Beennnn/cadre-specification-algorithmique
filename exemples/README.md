...
*Six exemples, délibérément différents. Aucun n'est une illustration décorative : chacun
...
|---|---|---|
| **[La vitesse moyenne d'un trajet](average-speed/)** | **Besoin → spécification → code → rapports** | La **prise en main** : deux fonctions, deux paramètres, trois règles, lu en cinq minutes. Et un vrai faux ami quand même — la vitesse moyenne n'est pas la moyenne des vitesses |
| **[Le bilan de masse d'un lot](mass-balance/)** | **Besoin → spécification → contrat → code → écarts** | La **chaîne complète**, seule du dépôt. Le code s'exécute, les données de référence sont rejouées, et l'analyse des écarts conclut que la spécification est en défaut |
...
| **[Le relevé d'une station météo](weather-summary/)** | Spécification seule, deux langues | **Tout le pseudo-langage** — 33 des 35 éléments du lexique dans un algorithme de 65 lignes : filtrage, groupement, tri, rejet **itératif** des valeurs aberrantes, table de décision. Et une **boucle qui peut s'épuiser** : la non-convergence n'est pas une erreur, c'est une étiquette de qualité |
...
**Si vous découvrez la méthode** — [le bilan de masse](mass-balance/), en suivant le
